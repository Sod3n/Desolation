    ...
    public static class AnimationProcessor
    {
        ...
        private static int ProcessClip(ModelImporterClipAnimation clip, IEnumerable<AnimationClipPreset> clipPresets)
        {
            foreach (var preset in clipPresets.OrderByDescending(p => p.priority))
            {
                ...
                bool fullNameFound = (!string.IsNullOrEmpty(preset.clipFullName) && clip.name == preset.clipFullName);
                bool partialNameFound = (preset.clipPartialPhrases.Any() && preset.clipPartialPhrases.All(s => clip.name.ToLowerInvariant().Contains(s.ToLowerInvariant())));
                bool defaultPreset = !preset.clipPartialPhrases.Any() && string.IsNullOrEmpty(preset.clipFullName);

                if (fullNameFound || partialNameFound || defaultPreset)
                {
